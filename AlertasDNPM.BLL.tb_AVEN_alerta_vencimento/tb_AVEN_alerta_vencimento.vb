Public Class tb_AVEN_alerta_vencimento
   ...
   Public Function GetNextIndex(ByVal dto As DTO.tb_AVEN_alerta_vencimento, field as String) As Long
        ...
        Dim dal As New DAL.tb_AVEN_alerta_vencimento(ConnectionString, dto)
    ...
    Public Function InsertData(ByVal dto As DTO.tb_AVEN_alerta_vencimento) As Boolean
    ...
    End Function
    Public Function UpdateData(ByVal dto As DTO.tb_AVEN_alerta_vencimento) As String
    ...
    End Function
    Public Function DeleteData(ByVal dto As DTO.tb_AVEN_alerta_vencimento) As String
    ...
    End Function
    Public Function GetData(ByVal dto As DTO.tb_AVEN_alerta_vencimento) As DataTable
    ...
    Public Function ConsultData(ByVal dto As DTO.tb_AVEN_alerta_vencimento) As DataTable
    ...
    Public Function PagingData(ByVal dto As DTO.tb_AVEN_alerta_vencimento) As DataTable
    ...
    Public Function Validate(ByVal dto As DTO.tb_AVEN_alerta_vencimento, _
        ...
        Dim dt As DataTable = fields.getFormData(_xmlPath, "AVEN")
...
FieldName = "AVEN_nm_tipo_documento"
ret = obj.ValidateString(fields.getValue(dt, FieldName), dto.AVEN_nm_tipo_documento, False, base._strPadrao, 10, True, getAllowedValues(FieldName), forceInvalidation, addFieldNameToMessage)
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret

FieldName = "AVEN_id_documento"
ret = obj.ValidateString(fields.getValue(dt, FieldName), dto.AVEN_id_documento, False, base._strPadrao, 20, True, getAllowedValues(FieldName), forceInvalidation, addFieldNameToMessage)
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret

FieldName = "AVEN_fl_conta_alerta"
ret = obj.ValidateNumeric(fields.getValue(dt, FieldName), dto.AVEN_fl_conta_alerta, Apoio.Validation.numericType._integer, False, base._intPadrao, base._intPadrao, True, getAllowedValues(FieldName), forceInvalidation, addFieldNameToMessage)
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret

FieldName = "AVEN_dt_ultimo_alerta"
ret = obj.ValidateDate(fields.getValue(dt, FieldName), dto.AVEN_dt_ultimo_alerta, Apoio.Validation.dateType._datetime, False, base._datPadrao, base._datPadrao, True, forceInvalidation, addFieldNameToMessage)
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret

FieldName = "AVEN_fl_ultimo_envio"
ret = obj.ValidateString(fields.getValue(dt, FieldName), dto.AVEN_fl_ultimo_envio, False, base._strPadrao, 1, True, getAllowedValues(FieldName), forceInvalidation, addFieldNameToMessage)
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret

If mode = tb_AVEN_alerta_vencimento.Mode.Insert Then 
...
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret
    ...
    Public Function customValidation(ByVal dto As DTO.tb_AVEN_alerta_vencimento) As String
        Dim ret As String = ""
        Dim val As New Apoio.Validation
        Dim obj As New Apoio.Globalization
        Dim dt As DataTable = obj.getData(_xmlPath, "customvalidate")
        'Dim dtFields As DataTable = obj.getFormData(_xmlPath, "AVEN")
    ...
    Public Function SaveData(ByVal dto As DTO.tb_AVEN_alerta_vencimento, _
                             ByRef message As String, _
                             ByRef action As ValidationReturnType, _
                             ByVal mode As Mode, _
        ...
        If boolValidate Then
            ...
            Try
             Select Case mode
                Case tb_AVEN_alerta_vencimento.Mode.Insert
                    Return InsertData(dto)
                Case tb_AVEN_alerta_vencimento.Mode.Update
...
'Case AVEN_nm_tipo_documento
 'values = ""

'Case AVEN_id_documento
 'values = ""

'Case AVEN_fl_conta_alerta
 'values = ""

'Case AVEN_dt_ultimo_alerta
 'values = ""

'Case AVEN_fl_ultimo_envio
 'values = ""
