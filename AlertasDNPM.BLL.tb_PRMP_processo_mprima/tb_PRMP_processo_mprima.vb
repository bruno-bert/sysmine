Public Class tb_PRMP_processo_mprima
   ...
   Public Function GetNextIndex(ByVal dto As DTO.tb_PRMP_processo_mprima, field as String) As Long
        ...
        Dim dal As New DAL.tb_PRMP_processo_mprima(ConnectionString, dto)
    ...
    Public Function InsertData(ByVal dto As DTO.tb_PRMP_processo_mprima) As Boolean
    ...
    End Function
    Public Function UpdateData(ByVal dto As DTO.tb_PRMP_processo_mprima) As String
    ...
    End Function
    Public Function DeleteData(ByVal dto As DTO.tb_PRMP_processo_mprima) As String
    ...
    End Function
    Public Function GetData(ByVal dto As DTO.tb_PRMP_processo_mprima) As DataTable
    ...
    Public Function ConsultData(ByVal dto As DTO.tb_PRMP_processo_mprima) As DataTable
    ...
    Public Function PagingData(ByVal dto As DTO.tb_PRMP_processo_mprima) As DataTable
    ...
    End Function
    Public Function PagingDataProd(ByVal dto As DTO.tb_PRMP_processo_mprima) As DataTable
        ...
        Dim dal As New DAL.tb_PRMP_processo_mprima(ConnectionString, dto)
        Return dal.PagingDataProd

    End Function


    Public Function Validate(ByVal dto As DTO.tb_PRMP_processo_mprima, _
        ...
        Dim dt As DataTable = fields.getFormData(_xmlPath, "PRMP")
...
FieldName = "PRMP_vl_mprima"
        ret = obj.ValidateNumeric(fields.getValue(dt, FieldName), dto.PRMP_vl_mprima, Apoio.Validation.numericType._decimal, True, 0, base._intPadrao, False, getAllowedValues(FieldName), forceInvalidation, addFieldNameToMessage)
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret

FieldName = "PRMP_un_mprima"
ret = obj.ValidateString(fields.getValue(dt, FieldName), dto.PRMP_un_mprima, False, base._strPadrao, 10, False, getAllowedValues(FieldName), forceInvalidation, addFieldNameToMessage)
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret

If mode = tb_PRMP_processo_mprima.Mode.Insert Then 
...
If ret.Trim.Length > 0 Then log = log & IIf(log.Trim.Length > 0, base._chrPulaLinhaGen, "") & ret
    ...
    Public Function customValidation(ByVal dto As DTO.tb_PRMP_processo_mprima) As String
        Dim ret As String = ""
        Dim val As New Apoio.Validation
        Dim obj As New Apoio.Globalization
        Dim dt As DataTable = obj.getData(_xmlPath, "customvalidate")
        'Dim dtFields As DataTable = obj.getFormData(_xmlPath, "PRMP")
    ...
    Public Function SaveData(ByVal dto As DTO.tb_PRMP_processo_mprima, _
                             ByRef message As String, _
                             ByRef action As ValidationReturnType, _
                             ByVal mode As Mode, _
        ...
        If boolValidate Then
            ...
            Try
             Select Case mode
                Case tb_PRMP_processo_mprima.Mode.Insert
                    Return InsertData(dto)
                Case tb_PRMP_processo_mprima.Mode.Update
