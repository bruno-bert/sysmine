...
Public Class PdfManipulation
        ...
        Try
            ...
            Else
                ...
                For i As Integer = 1 To n
                    outfile = baseNameOutPdf.Replace(ext, "_" & CStr(i) & ext)
                    doc = New iTextSharp.text.Document(reader.GetPageSizeWithRotation(currentPage))
                    pdfCpy = New iTextSharp.text.pdf.PdfCopy(doc, New IO.FileStream(outfile, IO.FileMode.Create))
                    doc.Open()
                    If i < n Then
    ...
    Public Sub AddPageNumberToPDFPages(ByVal file As String)

        Dim oldFile As String = file

        Dim newFile As String = Replace(file, "temp_", "")
        'Dim fsNew As New FileStream(newFile, FileMode.Create)
        ...
        Dim textStd As String = "Página {0} de {1}"
        Dim text As String = ""

        'Dim copy As PdfCopy = New PdfCopy(document, fsNew)
        

        Dim pageSize As Rectangle = document.PageSize
        Dim x As Integer = pageSize.GetLeft(520)
        Dim y As Integer = pageSize.GetBottom(30)
        ...
        For pageNum = 1 To totalPages
            ...
            cb.BeginText()
            text = String.Format(textStd, pageNum, totalPages)
    ...
    Public Sub MergePDFs(ByVal dt As DataTable, ByVal newFile As String)


        Dim document As New Document
        Dim fs As New FileStream(newFile, FileMode.Create)

        Dim copy As PdfCopy = New PdfCopy(document, fs)
        ...
        For Each dr As DataRow In dt.Rows
            ...
            reader = New PdfReader(dr("path").ToString.Trim)
